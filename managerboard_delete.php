<?php
    ...
    $sql = "select * from board_manager where num = $num"; // 데이터베이스에서 가져온 배열에서 파일 이름을 저장
    ...
    $sql = "delete from board_manager where num = $num"; // 게시글 번호에 해당하는 게시글을 삭제하는 쿼리 문자열 설정
    mysqli_query($con, $sql); // 데이터베이스에 쿼리를 실행하여 게시글을 삭제
    mysqli_close($con); // 데이터베이스 연결을 종료

    echo "
	     <script>
	         location.href = 'managerboard_list.php?page=$page'; // 게시글 목록 페이지로 이동
	     </script>
	   ";
?>
