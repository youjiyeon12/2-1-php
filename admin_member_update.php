<?php
    ...
    // 사용자 레벨이 관리자(레벨 1)이 아니면 수정할 수 없음
    if ($userlevel != 1) {
        echo("
            <script>
            alert('관리자가 아닙니다! 회원정보 수정은 관리자만 가능합니다!'); // 알림 메시지 출력
    ...
    $num   = $_GET["num"]; // GET 방식으로 전달된 회원 번호
    $level = $_POST["level"]; // POST 방식으로 전달된 레벨 정보


    $con = mysqli_connect("localhost", "user1", "12345", "music"); // 데이터베이스 연결
    $sql = "update members set level=$level where num=$num"; // 회원 정보를 수정하는 SQL 쿼리
    mysqli_query($con, $sql); // 쿼리 실행

    mysqli_close($con); // 데이터베이스 연결 종료

    // 회원 정보 수정 후 관리자 페이지로 이동하는 JavaScript 코드 출력
    echo "
	     <script>
