...
<script>
  function check_input() {
      if (!document.musicianboard_form.subject.value) // 제목이 입력되지 않았을 때
      {
          alert("제목을 입력하세요!"); // 알림창 표시
          document.musicianboard_form.subject.focus(); // 제목 입력 필드에 포커스 맞춤
          return;
      }
      if (!document.musicianboard_form.content.value) // 내용이 입력되지 않았을 때
      {
          alert("내용을 입력하세요!");    // 알림창 표시
          document.musicianboard_form.content.focus(); // 내용 입력 필드에 포커스 맞춤
          return;
      }
      document.musicianboard_form.submit(); // 제목과 내용이 모두 입력되었으면 폼 전송
   }
</script>
</head>
<body> 
...
<section>
	<div id="main_img_bar">
   	...
   	<div id="board_box">
	    <h3 id="board_title">
	    		뮤지션 게시판 > 글 쓰기 <!-- 게시판 글 쓰기 제목 -->
		</h3>
	    <form  name="musicianboard_form" method="post" action="musicianboard_insert.php" enctype="multipart/form-data"> <!-- 글 작성을 위한 폼 -->
	    	 <ul id="board_form">
				<li>
	    	...
	    	<ul class="buttons">
				...
				<li><button type="button" onclick="location.href='musicianboard_list.php'">목록</button></li> <!-- 목록으로 돌아가는 버튼 -->
			</ul>
	    </form>
	</div> <!-- board_box -->
</section> 
